...
async function readJson(response: Response) { const body = await response.text(); const data = body ? JSON.parse(body) : {}; if (!response.ok) throw new Error(firstText(record(data).msg, record(data).message, record(record(data).extras).error_message, body) || `카카오 API ${response.status}`); return data; }
function authHeaders(apiKey: string) { return { Authorization: `KakaoAK ${apiKey}`, "Content-Type": "application/json" }; }
...
export class KakaoChannelAdapter implements SalesChannelAdapter {
  async collectOrders(params: Record<string, unknown>): Promise<ChannelResult<NormalizedOrder[]>> {
    ...
    try {
      const search = new URLSearchParams({ size: "100", orderModifiedAtStart: formatKakaoDate(params.fromDate, "start"), orderModifiedAtEnd: formatKakaoDate(params.toDate, "end") });
      if (text(params.status)) search.set("orderStatus", text(params.status));
      const response = await fetch(`${KAKAO_BASE_URL}/v2/shopping/orders?${search.toString()}`, { headers: authHeaders(apiKey) });
      const data = await readJson(response);
      let rows = findRows(data);
      if (!rows.length && text(params.order_id)) {
        const detail = await readJson(await fetch(`${KAKAO_BASE_URL}/v1/shopping/order?order_id=${encodeURIComponent(text(params.order_id))}`, { headers: authHeaders(apiKey) }));
        rows = [record(detail)];
      }
