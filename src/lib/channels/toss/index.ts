...
function tossDateWindows(start: string, end: string) {
  const startMs = new Date(`${start}T00:00:00+09:00`).getTime();
  const endMs = new Date(`${end}T00:00:00+09:00`).getTime();
  if (!Number.isFinite(startMs) || !Number.isFinite(endMs) || startMs > endMs) return [{ start, end }];
  const dayMs = 24 * 60 * 60 * 1000;
  const format = (ms: number) => { const kst = new Date(ms + 9 * 60 * 60 * 1000); return `${kst.getUTCFullYear()}-${String(kst.getUTCMonth() + 1).padStart(2, "0")}-${String(kst.getUTCDate()).padStart(2, "0")}`; };
  const windows: Array<{ start: string; end: string }> = [];
  for (let cursor = startMs; cursor <= endMs && windows.length < 24; cursor += 31 * dayMs) {
    windows.push({ start: format(cursor), end: format(Math.min(cursor + 30 * dayMs, endMs)) });
  }
  return windows;
}
...
async function issueAccessToken(accessKey: string, secretKey: string, tokenUrl: string) {
  const cacheKey = `${tokenUrl}|${accessKey}`;
  const cached = tossTokenCache.get(cacheKey);
  ...
  const expiresInSeconds = num(data.expires_in || record(data.success).expires_in) || 3600;
  tossTokenCache.set(cacheKey, { token, expiresAt: Date.now() + Math.min(expiresInSeconds, 12 * 3600) * 1000 - 60_000 });
  return token;
}
function normalize(row: AnyRecord, base: { channelCode: string; channelName: string; customerCode?: string; customerName?: string }): NormalizedOrder {
  ...
  const item: NormalizedOrderItem = { channelProductCode: first(product.id, product.productId, row.productId, row.productNo), channelOptionCode: first(product.optionId, product.productItemId, row.orderProductId, row.optionId, row.stockId), channelProductName: first(product.name, row.productName, row.itemName, "토스 주문"), channelOptionName: first(product.optionName, row.optionName), sku: first(row.productItemManagementCode, product.productItemManagementCode, row.productManagementCode, product.productManagementCode, product.sellerProductCode, product.sellerItemCode, row.sku), qty: num(row.quantity || product.quantity || row.orderQuantity) || 1, salesAmount: num(row.paymentAmount || row.salesAmount || row.price || product.price || row.amount) || undefined, raw: row };
...
export class TossChannelAdapter implements SalesChannelAdapter {
  async collectOrders(params: Record<string, unknown>): Promise<ChannelResult<NormalizedOrder[]>> {
    ...
    try {
      ...
      // 토스 조회기간은 최대 31일이라 30일 단위로 나눠 조회한다.
      for (const window of tossDateWindows(startDate, endDate)) {
        const query = new URLSearchParams({ startDate: window.start, endDate: window.end, limit: "50", partnerName: text(params.partner_name) || "FNOS" });
  ...
  async confirmOrders(params: Record<string, unknown>): Promise<ChannelResult<unknown>> {
    ...
    try {
      ...
      // 토스 상태 변경은 1회 최대 100건
      for (let index = 0; index < ids.length; index += 100) {
        const body: AnyRecord = { orderProductIds: ids.slice(index, index + 100).map(tossId), status: "PREPARING_PRODUCT" };
        ...
        if (failedCount > 0) failureMessages.push(`${failedCount}건: ${first(success.failedReasons, record(data).error)}`);
        results.push(data);
      }
      if (failureMessages.length) return { ok: false, data: results, error: `토스 주문확인 일부 실패 ${failureMessages.join(" / ")}` };
      return { ok: true, data: results, message: `토스 주문확인 ${ids.length}건 요청 완료` };
