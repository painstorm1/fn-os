...
export class LotteonChannelAdapter implements SalesChannelAdapter {
  async collectOrders(params: Record<string, unknown>): Promise<ChannelResult<NormalizedOrder[]>> {
    ...
    try {
      ...
      const path = text(params.orders_path) || "/v1/openapi/order/v1/getSROrderList";
      const body = { srchStrtDttm: formatDate(params.fromDate ?? params.from, "start"), srchEndDttm: formatDate(params.toDate ?? params.to, "end"), lrtrNo: text(params.sub_partner_no || params.partner_no) };
      const response = await fetch(`${baseUrl}${path}`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: apiKey, "x-api-key": apiKey }, body: JSON.stringify(body) });
