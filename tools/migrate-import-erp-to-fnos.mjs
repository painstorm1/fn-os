...
async function ensureTargetTable(target, table, columns) {
  ...
  if (columns.some((column) => column.column_name === "id")) {
    await target.query(`create unique index if not exists ${qname(`${targetName}_id_uidx`)} on ${qname(targetName)} (id)`);
  }
}
...
async function upsertRows(target, table, columns, rows) {
  ...
  const updateColumns = columnNames.filter((name) => name !== "id");
  const conflictSql = columnNames.includes("id")
