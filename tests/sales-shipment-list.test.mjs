...
test("같은 품목도 일반과 직송은 두 행으로 유지한다", () => {
  ...
  assert.deepEqual(result.pages[0].left[25], { separator: true });
  assert.equal(result.pages[0].left[26], result.items[1]);
});
...
test("30개는 A5, 31개는 A4이다", () => {
  ...
  assert.equal(build(uniqueRows(30)).pages.length, 2);
  assert.equal(build(uniqueRows(30)).pages[0].left.length, 27);
  assert.equal(build(uniqueRows(31)).format, "A4");
});

test("A4는 45행씩 좌우 열 우선 배치하고 90/91에서 페이지가 나뉜다", () => {
  const onePage = build(uniqueRows(90));
  assert.equal(onePage.pages.length, 1);
  assert.equal(onePage.pages[0].left.length, 45);
  assert.equal(onePage.pages[0].right.length, 45);
  assert.equal(onePage.pages[0].left[0].productName, "품목 1");
  assert.equal(onePage.pages[0].right[0].productName, "품목 46");
  assert.equal(build(uniqueRows(91)).pages.length, 2);
});
...
test("직송 품목은 고정 그리드의 마지막 행부터 위로 배치된다", () => {
  ...
  assert.equal(result.pages[0].right[26].productName, "일반 72");
  assert.deepEqual(result.pages[0].right[43], { separator: true });
  assert.equal(result.pages[0].right[44].productName, "직송");
});
...
test("온라인발주 버튼과 팝업은 진행 단계 전체 행 및 SVG/인쇄/클립보드 경로를 사용한다", () => {
  ...
  assert.match(pageSource, /const topOffset = 75;/);
  assert.match(pageSource, /const rowHeight = 19;/);
  assert.match(popupSource, /result\.format === "A5" \? "A5 portrait" : "A4 portrait"/);
  assert.match(popupSource, /result\.format === "A5" \? "148mm" : "210mm"/);
  assert.match(popupSource, /result\.format === "A5" \? "210mm" : "297mm"/);
});
