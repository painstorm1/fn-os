...
test("같은 품목도 일반과 직송은 두 행으로 유지한다", () => {
  ...
  assert.deepEqual(result.pages[0].left[15], { separator: true });
  assert.equal(result.pages[0].left[16], result.items[1]);
});
...
test("30개는 A5, 31개는 A4이다", () => {
  ...
  assert.equal(build(uniqueRows(30)).pages[0].left.length, 17);
  assert.equal(build(uniqueRows(31)).format, "A4");
});

test("A4는 48행씩 좌우 열 우선 배치하고 96/97에서 페이지가 나뉜다", () => {
  const onePage = build(uniqueRows(96));
  assert.equal(onePage.pages.length, 1);
  assert.equal(onePage.pages[0].left.length, 48);
  assert.equal(onePage.pages[0].right.length, 48);
  assert.equal(onePage.pages[0].left[0].productName, "품목 1");
  assert.equal(onePage.pages[0].right[0].productName, "품목 49");
  assert.equal(build(uniqueRows(97)).pages.length, 2);
});
...
test("직송 품목은 고정 그리드의 마지막 행부터 위로 배치된다", () => {
  ...
  assert.equal(result.pages[0].right[23].productName, "일반 72");
  assert.deepEqual(result.pages[0].right[46], { separator: true });
  assert.equal(result.pages[0].right[47].productName, "직송");
});
