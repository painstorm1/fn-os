...
test("같은 품목도 일반과 직송은 두 행으로 유지한다", () => {
  ...
  assert.deepEqual(result.pages[0].left[13], { separator: true });
  assert.equal(result.pages[0].left[14], result.items[1]);
});
...
test("30개는 A5, 31개는 A4이다", () => {
  ...
  assert.equal(build(uniqueRows(30)).pages[0].left.length, 15);
