...
test("같은 품목도 일반과 직송은 두 행으로 유지한다", () => {
  ...
  assert.deepEqual(result.pages[0].left[12], { separator: true });
  assert.equal(result.pages[0].left[13], result.items[1]);
});
...
test("30개는 A5, 31개는 A4이다", () => {
  ...
  assert.equal(build(uniqueRows(30)).pages.length, 3);
  assert.equal(build(uniqueRows(30)).pages[0].left.length, 14);
  assert.equal(build(uniqueRows(31)).format, "A4");
});

test("A4는 46행씩 좌우 열 우선 배치하고 92/93에서 페이지가 나뉜다", () => {
  const onePage = build(uniqueRows(92));
  assert.equal(onePage.pages.length, 1);
  assert.equal(onePage.pages[0].left.length, 46);
  assert.equal(onePage.pages[0].right.length, 46);
  assert.equal(onePage.pages[0].left[0].productName, "품목 1");
  assert.equal(onePage.pages[0].right[0].productName, "품목 47");
  assert.equal(build(uniqueRows(93)).pages.length, 2);
});
...
test("직송 품목은 고정 그리드의 마지막 행부터 위로 배치된다", () => {
  ...
  assert.equal(result.pages[0].right[25].productName, "일반 72");
  assert.deepEqual(result.pages[0].right[44], { separator: true });
  assert.equal(result.pages[0].right[45].productName, "직송");
});
...
test("온라인발주 버튼과 팝업은 진행 단계 전체 행 및 SVG/인쇄/클립보드 경로를 사용한다", () => {
  ...
  assert.match(pageSource, /const topOffset = 150;/);
  assert.match(pageSource, /const rowHeight = 18;/);
});
