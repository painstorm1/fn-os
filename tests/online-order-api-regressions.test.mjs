...
function loadPageSsgCollectionHelpers() {
  const helperSource = `
    ...
    ${pageBlock("const orderCollectionApiDisplayChannels = [", "function onlineOrderStatusDisplaySiteKey(")}
    ${pageBlock("  function orderCollectionStatusItems(", "  async function revealOrderCollectionStatuses(")}
    module.exports = { appendCollectedOnlineOrdersToSheets, orderCollectionApiRunningItems, orderCollectionStatusItems };
  `;
...
test("주문수집 모달 API 영역은 최초·워커 대기·완료 모두 같은 7채널 순서를 유지한다", () => {
  const { orderCollectionApiRunningItems, orderCollectionStatusItems } = loadPageSsgCollectionHelpers();
  const expectedNames = ["네이버_에프엔FN", "네이버_펀앤파인", "쿠팡_WING", "11번가", "SSG신세계", "롯데온", "토스"];

  const initial = orderCollectionApiRunningItems("최근 2일");
  assert.deepEqual(initial.map((item) => item.name), expectedNames);
  assert.ok(initial.every((item) => item.status === "running" && item.message === "최근 2일"));

  const waiting = orderCollectionApiRunningItems("수집 대기 중", [
    { channel_code: "LOTTEON", channel_name: "롯데온 · 신규주문 수집", message: "롯데 대기" },
    { channel_code: "NAVER_FN", channel_name: "네이버_에프엔FN · 신규주문 수집", message: "네이버 대기" },
  ]);
  assert.deepEqual(waiting.map((item) => item.name), expectedNames);
  assert.equal(waiting.find((item) => item.name === "롯데온")?.message, "롯데 대기");
  assert.equal(waiting.find((item) => item.name === "토스")?.message, "수집 대기 중");

  const completed = orderCollectionStatusItems([
    { source: "api", channel_code: "COUPANG", channel_name: "쿠팡_WING · 신규주문 수집", order_status: "신규주문", ok: true, count: 3, item_count: 4 },
    { source: "api", channel_code: "LOTTEON", channel_name: "롯데온 · 주문확인 수집", order_status: "주문확인", ok: true, item_count: 2 },
  ], true, "");
  assert.deepEqual(completed.map((item) => item.name), expectedNames);
  assert.equal(completed.find((item) => item.name === "쿠팡_WING")?.message, "신규: 4건");
  assert.equal(completed.find((item) => item.name === "롯데온")?.message, "신규: 0건 / 주문확인: 2건");
  assert.equal(completed.find((item) => item.name === "토스")?.message, "신규: 0건");
});
