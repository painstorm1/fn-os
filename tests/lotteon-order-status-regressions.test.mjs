  ...
  const adapters = {
    LOTTEON: {
      dispatchOrders: async (params) => {
        captured.lotteon = params;
        return { ok: true, data: null, message: "lotte ok" };
      },
    },
    SSG: {
      dispatchOrders: async (params) => {
...
test("상태 route는 롯데ON 누락 배송사를 비워 두고 다른 채널 CJGLS fallback은 유지한다", async () => {
  const captured = {};
  ...
  assert.equal(captured.lotteon.dispatchProductOrders.length, 2, "롯데ON 2행 입력은 adapter의 fetch 전 단일행 gate가 볼 수 있도록 route에서 축소하면 안 됩니다.");
  assert.equal(captured.lotteon.dispatchProductOrders[0].deliveryCompanyCode, "");
  assert.equal(captured.ssg.dispatchProductOrders[0].deliveryCompanyCode, "CJGLS");
  assert.equal("spdNo" in captured.lotteon.dispatchProductOrders[0], false);
  assert.equal("sitmNo" in captured.lotteon.dispatchProductOrders[0], false);
});
