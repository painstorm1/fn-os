...
async function withFetchMock(mock, run, timerDelays) {
...
test("롯데ON CJ 0002 송장은 공백/하이픈만 제거하고 숫자가 아니면 fetch 전에 실패한다", async () => {
  const Adapter = loadLotteonAdapter();
  const calls = [];
  const result = await withFetchMock(async (url, init = {}) => {
    calls.push({ url: String(url), body: JSON.parse(String(init.body || "{}")) });
    if (calls.length === 1) return jsonResponse({ data: { rsltCd: "0000" } });
    return jsonResponse({ data: { deliveryProgressStateList: [{
      odNo: "OD-100", odSeq: 2, procSeq: 3, odPrgsStepCd: "13", invcNoList: ["1234567890"],
    }] } });
  }, () => new Adapter().dispatchOrders({
    api_key: "test-key",
    dispatchProductOrders: [validDispatchRow({ deliveryCompanyCode: "CJGLS", trackingNumber: "1234-567 890" })],
  }));

  assert.equal(result.ok, true);
  assert.deepEqual(calls[0].body.deliveryProgressStateList[0].invcNoList, ["1234567890"]);

  let invalidFetchCalls = 0;
  const invalid = await withFetchMock(async () => {
    invalidFetchCalls += 1;
    return jsonResponse({});
  }, () => new Adapter().dispatchOrders({
    api_key: "test-key",
    dispatchProductOrders: [validDispatchRow({ deliveryCompanyCode: "CJGLS", trackingNumber: "CJ-12345" })],
  }));
  assert.equal(invalid.ok, false);
  assert.match(invalid.error, /송장번호.*숫자/);
...
test("롯데ON API140 12→12→13은 bounded delay 후 성공하며 mutation은 재시도하지 않는다", async () => {
  const Adapter = loadLotteonAdapter();
  const calls = [];
  const timerDelays = [];
  const stages = ["12", "12", "13"];
  ...
  }, () => new Adapter().dispatchOrders({ api_key: "test-key", dispatchProductOrders: [validDispatchRow()] }), timerDelays);
  ...
  assert.deepEqual(calls.map((url) => url.includes("SellerDeliveryProgressStateInform") ? "API298" : "API140"), ["API298", "API140", "API140", "API140"]);
  assert.deepEqual(timerDelays, [1000, 3000]);
});

test("롯데ON API140 영구 stage12는 3회 delayed readback 후 실패한다", async () => {
  const Adapter = loadLotteonAdapter();
  const calls = [];
  const timerDelays = [];
  const result = await withFetchMock(async (url) => {
    calls.push(String(url));
    if (calls.length === 1) return jsonResponse({ data: { rsltCd: "0000" } });
    return jsonResponse({ data: { deliveryProgressStateList: [{ odNo: "OD-100", odSeq: 2, procSeq: 3, odPrgsStepCd: "12" }] } });
  }, () => new Adapter().dispatchOrders({ api_key: "test-key", dispatchProductOrders: [validDispatchRow()] }), timerDelays);
  ...
  assert.equal(calls.filter((url) => url.includes("SellerDeliveryProgressStateSearch")).length, 3);
  assert.deepEqual(timerDelays, [1000, 3000]);
  assert.match(result.error, /12|확인/);
});
