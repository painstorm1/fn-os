...
test("현대이지웰·ESM·오늘의집·카카오는 모든 파일 매핑행 출고완료 때만 cleanup 대상이다", async (t) => {
  const { applyOrderProgressStatusChangeToSheets } = loadManualCompletionHelpers();
  const previousWindow = globalThis.window;
  try {
    for (const { site, prefix } of [
      { site: "현대이지웰", prefix: "EZWEL" },
      { site: "ESM", prefix: "ESM" },
      { site: "오늘의집", prefix: "OHOU" },
      { site: "카카오", prefix: "KAKAO" },
    ]) {
      await t.test(site, () => {
        const fileName = `${prefix.toLowerCase()}-orders.xlsx`;
        const mallCodes = [`${prefix}-1`, `${prefix}-2`];
        globalThis.window = { localStorage: localStorageFixture({
          "fnos.salesInventory.onlineOrderManualFiles.v1": JSON.stringify([fileName]),
          "fnos.salesInventory.onlineOrderManualFileRows.v1": JSON.stringify({ [fileName]: mallCodes }),
        }) };
        const sheets = { "발주 진행 단계": mallCodes.map((mallCode) => [mallCode, "주문확인"]) };

        const partial = applyOrderProgressStatusChangeToSheets(sheets, [0], "출고완료");
        assert.deepEqual(partial.completedManualFiles, []);

        const complete = applyOrderProgressStatusChangeToSheets(sheets, [0, 1], "출고완료");
        assert.deepEqual(complete.completedManualFiles, [fileName]);
      });
    }
  } finally {
    globalThis.window = previousWindow;
